...
def test_view_profile(linkedin):
    err = linkedin.view_profile('venkchandran')
...
def test_get_company(linkedin):
    company = linkedin.get_company("northfield-global-consulting-llc")
    print(company)
    assert company
    assert company["name"] == "SvitSoft"
...
def test_search_people_by_keywords_filter(linkedin: Linkedin):
    fin_list = BIG_CO_LIST

    results, total_count = linkedin.search_people(
        connection_of='ACoAACd3X2UB2eDWFLquljMw2NtTpgp4PulsgEk',
...
def test_add_connection(linkedin):
    ...
    public_profile_id = 'joseph-w-kaupp-aba61351'
...
def test_connection_visibility_off(linkedin):
    response = linkedin.garner_connection_visibility(public_profile_id='cliff-simon')
